...
class CoderPasswordReset extends StatefulWidget {
  final Map arguments;
  const CoderPasswordReset({Key? key,required this.arguments}) : super(key: key);

  @override
  State<CoderPasswordReset> createState() => _CoderPasswordResetState();
}

class _CoderPasswordResetState extends State<CoderPasswordReset> {
  ...
  CoderRegistration coderRegistration = CoderRegistration();
        ...
        children: [
          ...
          TextButton(
            onPressed: ()async{
              ...
              showLoading(context);
              var res = await coderRegistration.changePassword(id: widget.arguments["id"], oldPassword: oldPassword.text, newPassword: pass1.text);
              if(res=="done"){
